...
- The new official **Android CLI** is installed in the toolchain audit and runtime-QA workflows from Google's stable package repository.
...
2. Runtime QA: API 37 emulator + Compose instrumentation tests.
3. Toolchain audit: weekly verification of JDK/SDK/CLI/Gradle alignment.
4. Release: signed APK/AAB, apksigner verification, checksum, and GitHub Release.
5. Security: dependency review, CodeQL, secret scanning, SBOM, and artifact attestation.
6. Visual QA: Android CLI `layout`, `screen`, screenshots, and journey-style smoke tests after the runtime path is stable.
