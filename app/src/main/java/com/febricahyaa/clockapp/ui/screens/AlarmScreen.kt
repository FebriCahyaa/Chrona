                    ...
                    ChronaCard(Modifier.fillMaxWidth(), glass = glass) {
                        Row(Modifier.fillMaxWidth().padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                            Column(Modifier.weight(1f)) {
                                Text(alarm.time.format(DateTimeFormatter.ofPattern("HH:mm")), fontSize = 35.sp, fontWeight = FontWeight.Light)
                            ...
                            IconButton(onClick = { onDelete(alarm) }) { Icon(Icons.Filled.DeleteOutline, null, tint = MaterialTheme.colorScheme.onSurfaceVariant) }
                        }
                    }
                }
...
private fun repeatLabel(days: Set<DayOfWeek>): String = when {
    ...
    else -> days.sortedBy { it.value }.joinToString("  ") { it.name.take(1) }
}
...
private fun AddAlarmDialog(onDismiss: () -> Unit, onAdd: (AlarmItem) -> Unit) {
    ...
    val labels = listOf(DayOfWeek.MONDAY to "M", DayOfWeek.TUESDAY to "T", DayOfWeek.WEDNESDAY to "W", DayOfWeek.THURSDAY to "T", DayOfWeek.FRIDAY to "F", DayOfWeek.SATURDAY to "S", DayOfWeek.SUNDAY to "S")
    AlertDialog(
