...
echo 'deb [arch=amd64 signed-by=/etc/apt/keyrings/google.asc] http://dl.google.com/android/cli/latest/debian/ stable main' | sudo tee /etc/apt/sources.list.d/android-cli.list >/dev/null
sudo apt-get update
